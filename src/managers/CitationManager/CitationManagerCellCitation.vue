<template>
	<TableCell :is-row-header="false">
		<div>
			...
			<div v-if="!hideRaw">{{ citation.rawCitation }}</div>
		</div>

		<div v-if="citation.rowExpanded">
			<div>
			...
			<div class="w-fit p-1" v-if="hideRaw">{{ citation.rawCitation }}</div>
		</div>
	</TableCell>
</template>
...
const hideRaw = computed(() => {
	return citationStore.isCitationStructured(citation.value);
});
</script>
